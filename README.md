...
5. bundle install
6. rails s 运行测试环境 
7. 如果使用自签名，可以使用：rails s -b 'ssl://ip:port?key=/path/privte.key&cert=/path/cert.crt' 来绑定证书运行 
8. 关于部署到生成环境的话请参照一下 rails puma 部署等教程，需要修改一下 config/deply.rb 的部署地址
9. 尝试访问 http://localhost:3000
```
